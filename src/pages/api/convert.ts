...
const generateAstroCode = (type: string, name: string, texts: string[], node: any) => {
  ...
  const exampleImage = type === "Gallery" ? `<img src="https://via.placeholder.com/150" alt="example" />` : "";
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const res = await fetch(`https://api.figma.com/v1/files/${fileKey}`, { headers: { "X-Figma-Token": FIGMA_TOKEN } });
    ...
    allFrames.forEach(f => {
      ...
      if (["Card", "Blog", "Testimonial"].includes(type)) {
        if (duplicates.has(f.name)) return;
        duplicates.add(f.name);
      }
      const texts = collectTextNodes(f);
      const uniqueName = getUniqueName(type);
      sections.push({ id: f.id, name: f.name, component: uniqueName, code: generateAstroCode(type, uniqueName, texts, f) });
    });
